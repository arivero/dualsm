...
\begin{enumerate}[label=(\roman*)]
  ...
  \item \textbf{Electric--magnetic duality.}  SUSY is the \emph{only}
    ...
    and weakly-coupled gauge theories can be \emph{established through exact consistency checks}---anomaly matching, moduli space matching, and
...
\begin{remark}[Strategy for the course]
  ...
  (this lecture); (2)~\emph{derive evidence for} Seiberg duality in the supersymmetric
...
\noindent\textbf{Baryon number convention.}\; In this table we adopt
$B[Q] = +1$, $B[\widetilde{Q}] = -1$ for the SQCD field content,
differing from the Seiberg convention $B[Q] = 1/\Nc$ used in Lecture~1.
The two are related by rescaling $B \to \Nc\, B$; all anomaly matching
conditions remain valid provided the same convention is used
...
All dimensions below are \textbf{complex} unless stated otherwise.

The \textbf{dimension} of the classical moduli space is:
\begin{equation}
  \boxed{
    \dim_{\mathbb{C}}(\mathcal{M}_{\text{cl}})
    = 2\Nc \Nf - (\Nc^2 - 1)\,,
  }
  \label{eq:moduli-dim}
\end{equation}
which counts the $2\Nc\Nf$ complex scalar DOF in $(Q, \widetilde{Q})$
(each chiral superfield contains one complex scalar) minus the
$\Nc^2 - 1$ complex parameters of $\SU{\Nc}_{\mathbb{C}}$ gauge
transformations (the complexified gauge group acts on the chiral fields
holomorphically).

\begin{remark}[Consistency check: $\Nf = \Nc = 2$]
The formula gives $\dim_{\mathbb{C}} = 2 \times 2 \times 2 - 3 = 5$.
We can verify this from gauge-invariant coordinates: the $2 \times 2$
meson matrix $M$ has 4 complex entries, plus the baryons $B$ and
$\widetilde{B}$ (1 complex each), giving $4 + 1 + 1 = 6$ complex
gauge-invariant variables.  Imposing the classical constraint
$\det M - B\,\widetilde{B} = 0$ (one complex equation) removes one
complex degree of freedom, leaving $6 - 1 = 5$ complex dimensions,
in agreement with~\eqref{eq:moduli-dim}.
\end{remark}
