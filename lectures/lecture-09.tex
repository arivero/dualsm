...
The Kutasov--Schwimmer family is important because it shows that Seiberg
duality generalises to theories with adjoint matter, and the $k=1$ limit
recovers basic Seiberg duality as a special case.  This extension is
directly relevant to the dualised SM, whose electric theory contains an
adjoint Weyl fermion (Lecture~7).
...
The $\SO{\Nc}$ family demonstrates that electric--magnetic duality is not
special to unitary gauge groups.  Because the fundamental of $\SO{\Nc}$
is real, the global symmetry structure and the form of the dual theory
differ qualitatively from the $\SU{\Nc}$ case.
...
The $\Sp{\Nc}$ family is the pseudo-real counterpart of the $\SO{\Nc}$
dualities.  It plays a distinguished role in the dualised SM programme:
the electroweak $\SU{2}_L \cong \Sp{1}$ sector of the Standard Model
can be viewed as the simplest symplectic gauge theory, and the $\Sp{1}$
duality of Paper~1 exploits precisely this structure.
...
s-Confining theories are important because they provide a class of exact
low-energy descriptions with \emph{no residual gauge dynamics}: the IR
theory is a Wess--Zumino model of gauge-singlet composites.  They serve
as endpoints of mass deformation chains (e.g.\ $\Nf = \Nc + 1$ in
$\SU{\Nc}$ SQCD) and as building blocks for model-building.
...
Product group dualities extend Seiberg duality from simple gauge groups
to quiver gauge theories---the natural setting for string-motivated
model-building and for theories with multiple gauge sectors such as the
...
The duality cascade is important because it shows how iterated Seiberg
duality can generate a logarithmic flow between energy scales---an RG
staircase in which the effective gauge group rank decreases at each
step.  This provides the clearest link between Seiberg duality and
