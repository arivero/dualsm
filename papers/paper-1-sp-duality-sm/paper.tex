...
verify 't~Hooft anomaly matching, identify the composite meson
content with Higgs doublets, and check the conformal window
constraints.  For three Standard Model generations, the
electroweak sector contains $2\Nf = 12$ Weyl doublets of $\SU{2}_L$,
corresponding to $\Nf = 6$ in the Sp counting convention.  The
electric dual is then $\Sp{3}$ with 12~fundamentals, sitting at the
lower boundary of the conformal window.  The antisymmetric meson
$M_{ij}$ yields $\binom{12}{2} = 66$ real scalar degrees of freedom
that decompose into Higgs doublets.  We compare this Sp route with
the $\SU{N}$ Cacciapaglia--Sannino construction and find structurally
distinct predictions: a different meson symmetry (antisymmetric vs.\
bifundamental), a different generation bound, and a different number
of composite Higgs doublets.  All claims involving the extrapolation
of SUSY Seiberg duality to the non-supersymmetric Standard Model are
conjectural.
\end{abstract}
...
\begin{enumerate}
  ...
  \item The antisymmetric meson $M_{ij} = -M_{ji}$ yields
    $\Nf(2\Nf - 1) = 66$ real scalar components, which decompose
    under $\SU{2}_L$ into Higgs-like multiplets
