  ...
  \item \textbf{[8 marks]}
    ...
    \begin{enumerate}[label=(\roman*)]
      \item For the electric $\SU{2}$ gauge theory underlying the SM
        ...
        $T(\adj) = N_c^e = 2$.
        Argue that the electric theory is \emph{not} asymptotically
        free (or marginally so), which is consistent with the duality
        requirement that the electric theory becomes strongly coupled
        in the infrared.

      \item The statement that $\alpha_e(\Lambda_D) \sim \mathcal{O}(1)$
        (strong coupling) implies via
        Eq.~\eqref{eq:duality-coupling} that
        $\alphas(\Lambda_D) \sim 1/\alpha_e(\Lambda_D)$.  For a
        reasonable input $\alphas(\Lambda_D) = 0.3$, use
        Eq.~\eqref{eq:lambda-qcd-def} with $\mu_0 = \Lambda_D
        = 10^{11}\;\text{GeV}$ and $\bzero = 7$ to compute
        $\Lambda_{\QCD}$ numerically.  Show all intermediate steps.
  ...
  \item \textbf{[5 marks]}
    ...
    \begin{enumerate}[label=(\roman*)]
      \item Complete the following input/output table for the dualised
        ...
        $\alphas(\Lambda_D)$ & \rule{3cm}{0.4pt} & $\sim 0.3$ \\
...
\begin{itemize}
  \item For (a)(ii): set the denominator of
    ...
    $\mu$.
  \item For (b)(ii): compute $2\pi/(\bzero \cdot \alphas)$
    first, then take the exponential.
