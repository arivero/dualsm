...
$\Delta = \tfrac{3}{2}\,R$ (for operators with $R > 0$, which holds for
all gauge-invariant chiral primaries in the conformal window).
...
(c) The mesons $M = Q\widetilde{Q}$ are composites of two dimension-1
fields, so $[M] = [\mathrm{mass}]^2$ (not $[\mathrm{mass}]$; see the
solution for the full dimensional analysis).
