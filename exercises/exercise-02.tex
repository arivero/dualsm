...
that the electric conformal description breaks down and the IR is better described by the magnetic (confined/free-magnetic) variables.
...
The SUSY conformal window (shaded orange, exact: $3\Nc/2 < \Nf < 3\Nc$)
is shown alongside the non-SUSY estimates (blue/green regions, approximate).
\emph{These are distinct theories with different dynamics.}
The horizontal axis counts Dirac flavours in both cases, but the SUSY
result applies to $\mathcal{N}=1$ SQCD (with superpartners), while the
non-SUSY boundaries ($\Nf^*$ and the Banks--Zaks regime) refer to ordinary
QCD-like theories.  The SUSY window is overlaid to show where exact results
are available; one should \emph{not} read it as a subregion of the non-SUSY
phase diagram.
