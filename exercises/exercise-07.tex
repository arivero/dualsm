  ...
  \item \textbf{[5 marks]}
    ...
    \begin{enumerate}[label=(\roman*)]
      \item Verify that $W_{\text{conf}}$ has the correct mass
        ...
        The fields $M$, $B$, $\widetilde{B}$ are composites with
        standard canonical mass dimensions: $[M] = [Q\,\widetilde{Q}]
        = [\text{mass}]^2$, $[B] = [Q^{\Nc}] = [\text{mass}]^{\Nc}$,
        $[\widetilde{B}] = [\widetilde{Q}^{\Nc}]
        = [\text{mass}]^{\Nc}$.  What is the
