...
the $\mathcal{N}=1$ values $R[Q] = (\Nf - \Nc)/\Nf$.  Even at
$\Nf = 2\Nc$ (the $\mathcal{N}=2$ conformal point), the two
assignments do not coincide: the $\mathcal{N}=1$ formula gives
$R[Q] = 1/2$, while $\mathcal{N}=2$ assigns $R[Q] = 1$.  This is
because the anomaly-free $\mathcal{N}=1$ $R$-symmetry is a specific
linear combination of the $\mathcal{N}=2$ $R$-charges and the Cartan
generator of the $\mathrm{SU}(2)_R$ symmetry of the extended algebra.
When $\mathcal{N}=2$ is broken to $\mathcal{N}=1$ (e.g.\ by the APS
mass deformation), the $\mathrm{SU}(2)_R$ is explicitly broken and
only the $\mathcal{N}=1$ combination survives as a symmetry.
\end{remark}
...
(this descent is established within the broader Seiberg--Witten/APS framework, not derived from first principles within this exercise).
