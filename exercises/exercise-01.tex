...
\noindent\textbf{Questions.}
\begin{enumerate}[label=(\alph*)]
  \item Compute the \UV{} anomaly coefficients: $\SU{2}_L^3$, $\SU{2}_R^3$,
    $\SU{2}_L^2\,\UU{1}_B$, $\UU{1}_B^3$, $\UU{1}_B$-gravitational.
...
The $\SU{2}_L^3$ anomaly is non-zero in the \UV{}.  What fermion can
match it?
...
\underline{$\SU{2}_L^3$:} Only $q_L$ transforms under $\SU{2}_L$.  There
are $\Nc = 3$ colour copies of the fundamental of $\SU{2}_L$:
\begin{equation}
  \mathcal{A}_{\UV}[\SU{2}_L^3] = \Nc \times A(\fund_{\SU{2}}) = 3 \times 1 = 3.
\end{equation}
(We use $A(\fund) = 1$.)

\underline{$\SU{2}_R^3$:} By $L \leftrightarrow R$ symmetry:
\begin{equation}
...
$\SU{2}_L^3$ anomaly is $3$ in the \UV{}, so there must be massless
\emph{fermions} in the \IR{} to produce a non-zero anomaly coefficient.
...
\underline{$\SU{2}_L^3$:} Only $N_L$ contributes (it is the only field
with $\SU{2}_L$ charge):
\begin{equation}
  \mathcal{A}_{\IR}[\SU{2}_L^3]
  = 1 \times A(\fund_{\SU{2}})
  = 1.
\end{equation}

This gives $\mathcal{A}_{\IR} = 1$, but $\mathcal{A}_{\UV} = 3$.
The matching fails!

\textbf{Resolution:} For $\SU{2}$ specifically, $A(\fund_{\SU{2}}) = 1$,
but $\SU{2}$ has no cubic anomaly: $d^{abc} = 0$ for $\SU{2}$.  The
purely cubic anomaly $\SU{2}^3$ actually vanishes identically for all
representations of $\SU{2}$, because $\SU{2}$ has only pseudo-real
representations.

Therefore, the $\SU{2}_L^3$ anomaly matching condition is trivially
$0 = 0$ for both \UV{} and \IR{}.  This is a peculiarity of $\SU{2}$:
the non-trivial anomaly matching constraints come from the mixed anomalies,
not the purely cubic ones.

\medskip

Let us redo the analysis using the \emph{non-trivial} anomaly matching
conditions.  For $\SU{2}$, the meaningful anomalies to match are those
involving the Witten $\SU{2}$ global anomaly (discrete, $\pi_4(\SU{2}) = \mathbb{Z}_2$)
and the mixed anomalies $\SU{2}^2\,\UU{1}$.
